...
public class AutoLoader extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (intent.getAction().equals("android.intent.action.BOOT_COMPLETED")) {
			...
			String AktuellesDatum = DateFormat.getDateInstance().format(
					new Date());
			String AktuelleUhrzeit = DateFormat.getTimeInstance().format(
					new Date());
			...
			if (getLastBoot(context) != AktuellesDatum) {
				...
				try {
					...
					if (aktuellesDatum.getDayOfMonth() == 1) {
						...
						Cursor c = BudgetLoader.getDetailsOfSparziele(dbRead,
								context);

						while (c.moveToNext()) {

							Integer id = BudgetLoader.getNewTransaktionID(
									context, dbRead);

							BudgetLoader.addEinmaligeTransaktion(dbWrite,
									context, id, "", AktuellesDatum,
									AktuelleUhrzeit, c.getString(0),
									c.getString(2));

							BudgetLoader.addCreditToSparziel(dbWrite, dbRead,
									context, Integer.parseInt(c.getString(2)),
									c.getString(0));
						}
					...
					Cursor cT = BudgetLoader.getDetailsOfMonthlyTransaktionen(
							dbRead, context);

					while (cT.moveToNext()) {
						// wenn am heutigen tag im monat eine Transaktion zu
						// buchen ist
						if (Integer.parseInt(cT.getString(1)) == aktuellesDatum
								.getDayOfMonth()) {
							...
							BudgetLoader.addEinmaligeTransaktion(dbWrite,
									context, id, "", AktuellesDatum,
									AktuelleUhrzeit, cT.getString(0),
									cT.getString(2));
					...
					if (aktuellesDatum.getDayOfMonth() == 1) {
						...
						Cursor cK = BudgetLoader.getDetailsOfKategorien(dbRead,
								context);

						while (cK.moveToNext()) {
							BudgetLoader.resetRestbudgetsKategorien(dbWrite,
									context, cK.getString(0), cK.getString(2));
