...
import einstellungen.EinstellungenActivity;
...
public class AusgabeActivity extends BaseActivity {
	...
	protected void onStart() {
		...
		String AktuelleUhrzeit = DateFormat.getTimeInstance()
				.format(new Date());
	...
	public boolean addAusgabe(View view) {
		...
		boolean checkKategorie = BudgetLoader.checkIfKategorieEnoughMoney(
				getApplicationContext(), kategorieTitel, betragPre);
		Integer checkGesamt = BudgetLoader
				.getSumOfTransaktionen(getApplicationContext());
		...
		if (checkGesamt < betragPre) {
			...
			alert.setNegativeButton("Trotzdem fortfahren",
					new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog, int id) {
							addAusgabeToTransaktion(id, anmerkungFeld.getText()
									.toString(),
									datumFeld.getText().toString(), zeitFeld
											.getText().toString(),
			...
			AlertDialog.Builder alert = new AlertDialog.Builder(this);
			alert.setMessage("Restbudget der Kategorie " + kategorieTitel
					+ " nicht ausreichend.");
			alert.setNegativeButton("Trotzdem fortfahren",
					new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog, int id) {
			...
			try {
				...
				addAusgabeToTransaktion(id, anmerkungFeld.getText().toString(),
						datumFeld.getText().toString(), zeitFeld.getText()
								.toString(), kategorieSpin.getSelectedItem()
								.toString(), betrag);
	...
	private void addAusgabeToTransaktion(Integer id, String anmerkung,
			String datum, String zeit, String kategorie, String betrag) {
		// Einmalige Ausgabe in DB-Tabelle schreiben
		BudgetLoader.addEinmaligeTransaktion(getApplicationContext(), id,
				anmerkung, datum, zeit, kategorie, betrag);

		// wenn kategorie 'ohne kategorie' nicht ausgewaehlt worden ist
		if (kategorieSpin.getSelectedItem().toString() != "ohne Kategorie") {
			BudgetLoader.addEinmaligeTransaktionToKategorie(
					getApplicationContext(), datum, kategorie, betrag);
		}
	...
	// Wenn button pressed alle Eingaben zuruecksetzen und in MainActivity
	// wechseln
