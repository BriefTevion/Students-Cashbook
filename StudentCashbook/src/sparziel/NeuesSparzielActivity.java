...
public class NeuesSparzielActivity extends BaseActivity {
	...
	public void zielAnlegen(View view) {
		...
		if (titelFeld.getText().toString().isEmpty()
				|| datumFeld.getText().toString().isEmpty()
				|| betragFeld.getText().toString().isEmpty()) {
			...
			try {
				zielDatum = new DateMidnight(formatedDate
						+ "T00:00:00.000+01:00");
			} catch (Exception e) {
				Log.v("test", e.getMessage());
			}
			// Monate zwischen den Daten bekommen
			int diffMonths = Months.monthsBetween(aktuellesDatum, zielDatum)
					.getMonths();

			// wenn zu der monatliche Sparbetrag mal der Laufzeit
			// kleiner ist als der Zielbetrag, muss eine Warnung angezeigt
			// werden
			if (diffMonths * Integer.parseInt(sparBetrag) < Integer
					.parseInt(betragFeld.getText().toString())) {

				int mindSparbetrag = Integer.parseInt(betragFeld.getText()
						.toString()) / diffMonths;
				...
				try {
					...
					BudgetLoader.addSparziel(getApplicationContext(), titelFeld
							.getText().toString(), betragFeld.getText()
							.toString(), datumFeld.getText().toString(),
							sparBetrag);
